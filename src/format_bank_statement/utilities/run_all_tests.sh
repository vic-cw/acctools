...
Usage: $SCRIPT_NAME [-h]
       ...
       Run all tests from format_statement/utilities folder, 
...
"$_DIR_/identify_statement/run_all_tests.sh" || \
                ALL_GOOD=false
"$_DIR_/convert_cic_pdf_text_to_csv/run_all_tests.sh" || \
		ALL_GOOD=false 
...
if $ALL_GOOD; then
	printf $green
	echo "--"
	echo "All format_statement/utilities tests passed successfully"
	echo $end_color
	exit 0
else
	printf $red
	echo "--"
	echo "Some format_statement/utilities tests failed" $end_color
	exit 1
fi
